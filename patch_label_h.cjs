...
const replacement = `                      <div className="absolute right-0 top-[-5px] flex flex-col items-end shrink-0">
                          <div className="scale-x-[1.3] origin-top-right">
                              <Barcode 
                                value={String(printData.job.osNumber || printData.job.id.substring(0,8))} 
                                width={1} 
...
if (code.includes(target)) {
    ...
    console.log("Patched PrintOverlay.tsx with requested height and top margin");
} else {
