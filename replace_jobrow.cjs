...
const startIndex = content.indexOf('const JobRow = memo(');
...
if (startIndex === -1 || endIndex === -1) {
    console.log('Could not find JobRow boundaries.');
    process.exit(1);
}

const replacement = `const JobRow = memo(({ isJobOverdue, 
    job, 
    isClient,
    isBudgetMode, 
    isLabStaff, 
    ...
    return (
        <tr className={\`hover:bg-blue-50/30 transition-colors \${showAttention ? 'bg-yellow-50/50' : ''}\`}>
            ...
            {isBudgetMode && (
                <td className="p-4">
                    <div className="text-xs font-bold text-slate-900 uppercase tracking-tight">{job.patientName}</div>
                </td>
            )}
            {!isBudgetMode && (
