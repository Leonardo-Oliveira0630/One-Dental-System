...
const targetStr = `        chronoHistory.history.forEach((rawItem) => {
...
const replaceStr = `        chronoHistory.history.forEach((rawItem: any) => {
            ...
            const jobObj = 'job' in item ? (item as any).job : undefined;
            const hasSubItems = isDebit && jobObj && jobObj.items && jobObj.items.length > 0;
            
            let description = '';
            if (isDebit) {
                const dentistName = (statementClient.name && statementClient.name.split(' ')[0]) || 'Dr.';
                description = \`\${jobObj?.osNumber || '-'} - Dr(a): \${dentistName.toUpperCase()} - Paciente: \${(jobObj?.patientName || '').toUpperCase()}\`;
            } else {
                description = item.description;
            }
            ...
            if (hasSubItems && jobObj) {
                jobObj.items.forEach((subItem: any, subIndex: number) => {
                    const isLast = subIndex === jobObj.items.length - 1;
                    tableBody.push([
                        { content: '', styles: { lineWidth: { bottom: isLast ? 0.1 : 0 } as any, lineColor: [220,220,220] } },
                        { content: \`\${subItem.quantity}      \${subItem.name.toUpperCase()}\`, styles: { textColor: [100,100,100], fontSize: 8, lineWidth: { bottom: isLast ? 0.1 : 0 } as any, lineColor: [220,220,220] } },
                        { content: \`R$ \${(subItem.price * subItem.quantity).toLocaleString('pt-BR', {minimumFractionDigits: 2})}\`, styles: { textColor: [100,100,100], fontSize: 8, lineWidth: { bottom: isLast ? 0.1 : 0 } as any, lineColor: [220,220,220] } },
...
console.log('Patched Dentists.tsx type error job pdf12');
