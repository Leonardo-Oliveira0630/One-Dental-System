...
if (startIdx !== -1 && endIdx !== -1) {
    ...
    // Insert new block
    const splitPointStr1 = '                      </div>\n                    )}\n                </div>\n            </div>';
    const splitPointStr2 = '                      </div>\n                    )}\n                </div>\n            </div>'; // Try finding where lg:col-span-8 ends.
    
    const idx2 = code.indexOf('            </div>\n            \n            <div className="lg:col-span-4');
    const idx3 = code.indexOf('            </div>\n                        \n            <div className="lg:col-span-4');
    
    let insertionIdx = -1;
    if (idx3 !== -1) insertionIdx = idx3;
    else if (idx2 !== -1) insertionIdx = idx2;
    
    if (insertionIdx !== -1) {
        code = code.substring(0, insertionIdx) + materiaisBlock + code.substring(insertionIdx);
        fs.writeFileSync('pages/NewJob.tsx', code);
        console.log("Success moving in NewJob");
    } else {
        console.log("Could not find insertion point by index");
    }
} else {
