...
export const Dashboard = () => {
          ...
          <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500 w-full overflow-hidden text-slate-800 dark:text-slate-100">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="min-w-0">
                    <h1 className="text-2xl md:text-3.5xl font-bold text-slate-900 dark:text-white tracking-tight truncate">{t('dashboard.helloDentist', 'Olá, Dr(a).')} {currentUser.name}</h1>
                    <p className="text-[10px] md:text-sm text-slate-500 dark:text-slate-400 font-medium uppercase tracking-widest mt-1 truncate">{t('dashboard.dentistSubtitle', 'Acompanhamento e solicitações em tempo real')}</p>
                  </div>
                  <button onClick={() => navigate('/store')} className="w-full md:w-auto flex items-center justify-center gap-2 px-6 py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-card shadow-premium hover:shadow-xl transition-all duration-300 uppercase text-xs tracking-wider shrink-0"><ShoppingBag size={18} /> {t('dashboard.newWebOrder', 'Novo Pedido Web')}</button>
              </div>
              ...
              {!activeOrganization ? (
                  <div className="bg-orange-50 dark:bg-orange-950/20 border-2 border-dashed border-orange-200 dark:border-orange-900/40 p-6 md:p-10 rounded-card flex flex-col md:flex-row items-center justify-between gap-4 sm:p-6 transition-all">
                      <div className="flex flex-col md:flex-row items-center gap-4 text-center md:text-left min-w-0">
                          ...
                          <div className="min-w-0">
                              <h3 className="text-lg font-semibold text-orange-950 dark:text-orange-200 uppercase tracking-wider mb-1">{t('dashboard.noPartnershipTitle', 'Sem Parceria Selecionada')}</h3>
                              <p className="text-orange-700 dark:text-orange-300 text-sm font-medium">{t('dashboard.noPartnershipDesc', 'Selecione ou adicione um laboratório parceiro para visualizar produtos e fazer pedidos.')}</p>
                          </div>
                      </div>
                      <button onClick={() => navigate('/dentist/partnerships')} className="w-full md:w-auto px-8 py-3.5 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-lg shadow-lg shadow-orange-100 dark:shadow-none uppercase text-xs shrink-0 transition-colors">{t('dashboard.connectNow', 'Conectar agora')}</button>
                  </div>
              ) : (
                  <div className="bg-gradient-to-br from-[#0F4C81] to-[#0A365C] dark:from-[#0c2e4e] dark:to-[#081b2e] text-white p-6 md:p-10 rounded-card shadow-premium border border-slate-700/40 flex flex-col md:flex-row items-center justify-between gap-4 sm:p-6 relative overflow-hidden">
                      ...
                      <div className="flex-1 relative z-10 text-center md:text-left min-w-0">
                          <div className="flex items-center justify-center md:justify-start gap-2 text-[#00B8D9] font-bold uppercase text-[10px] tracking-widest mb-3 truncate">
                            ...
                            {t('dashboard.activeLab', 'Laboratório Ativo')}
                          </div>
                          <h2 className="text-2xl md:text-4.5xl font-bold mb-2 tracking-tight truncate text-white">{activeOrganization.name}</h2>
                          <p className="text-slate-100/90 text-sm font-normal">{t('dashboard.trackingJobs', { count: jobs.length, defaultValue: `Acompanhando ${jobs.length} pedidos em andamento neste parceiro.` })}</p>
                      </div>
                      <div className="flex flex-col xs:flex-row gap-3 w-full md:w-auto relative z-10">
                          <button onClick={() => navigate('/jobs')} className="flex-1 px-6 py-3.5 bg-white/10 hover:bg-white/20 border border-white/10 font-bold rounded-input transition-all uppercase text-[10px] tracking-widest truncate text-white">{t('dashboard.myOrders', 'Meus Pedidos')}</button>
                          <button onClick={() => navigate('/store')} className="flex-1 px-8 py-3.5 bg-white hover:bg-slate-50 text-[#0F4C81] font-bold rounded-input shadow-soft transition-all hover:scale-[1.02] uppercase text-[10px] tracking-widest truncate">{t('dashboard.browseCatalog', 'Consultar Catálogo')}</button>
                      </div>
                  </div>
              )}

              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-4 sm:p-6">
                  <StatCard title={t('dashboard.inProduction', 'Em Produção')} value={totalActive} icon={<Activity size={20} className="text-[#00B8D9]" />} color="bg-[#00B8D9]/10 dark:bg-[#00B8D9]/20" />
                  <StatCard title={t('dashboard.waitingApproval', 'Aguardando Aprovação')} value={jobs.filter(j => j.status === JobStatus.WAITING_APPROVAL).length} icon={<Clock size={20} className="text-[#2F80ED]" />} color="bg-[#2F80ED]/10 dark:bg-[#2F80ED]/20" />
                  <StatCard title={t('dashboard.completed', 'Concluídos')} value={jobs.filter(j => j.status === JobStatus.COMPLETED).length} icon={<CheckCircle size={20} className="text-[#10B981]" />} color="bg-[#10B981]/10 dark:bg-[#10B981]/20" />
                  <StatCard title={t('dashboard.urgencies', 'Urgências')} value={urgent} icon={<AlertTriangle size={20} className="text-[#F59E0B]" />} color="bg-[#F59E0B]/10 dark:bg-[#F59E0B]/20" />
              </div>
          </div>
      );
  }
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500 w-full overflow-hidden text-slate-800 dark:text-slate-100">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="min-w-0">
          <h1 className="text-2xl md:text-3.5xl font-bold text-slate-900 dark:text-white tracking-tight">{t('dashboard.title', 'Painel de Controle')}</h1>
          <p className="text-[10px] md:text-sm text-slate-500 dark:text-slate-400 font-medium uppercase tracking-widest mt-1">{t('dashboard.subtitle', 'Visão Geral da Produção e Indicadores em Tempo Real')}</p>
        </div>
        <button onClick={handleGenerateInsights} disabled={loadingAi} className="w-full md:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-card font-semibold shadow-premium hover:shadow-xl transition-all disabled:opacity-70 text-xs tracking-wider uppercase shrink-0"><Sparkles size={16} className={loadingAi ? "animate-spin shrink-0" : "shrink-0 text-cyan-300"} /> {loadingAi ? t('dashboard.aiAnalyzing', 'Analisando...') : t('dashboard.aiInsights', 'Insights com IA')}</button>
      </div>
      ...
      {aiInsights && (
        <div className="bg-gradient-to-r from-blue-950/40 to-cyan-950/20 border border-l-4 border-l-blue-500 border-blue-500/30 rounded-card p-4 sm:p-6 md:p-4 sm:p-8 relative overflow-hidden animate-in zoom-in duration-300">
            ...
            <h3 className="text-xs md:text-sm font-bold text-blue-600 dark:text-blue-400 mb-3 flex items-center gap-2 uppercase tracking-widest"><Sparkles size={16} className="text-cyan-400" /> {t('dashboard.aiReport', 'Relatório Estratégico AI')}</h3>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-4 sm:p-6">
        <StatCard title={t('dashboard.activeProduction', 'Produção Ativa')} value={totalActive} icon={<Activity size={20} className="text-[#00B8D9]" />} color="bg-[#00B8D9]/10 dark:bg-[#00B8D9]/20" />
        <StatCard title={t('dashboard.readyToday', 'Prontos Hoje')} value={completedToday} icon={<CheckCircle size={20} className="text-[#10B981]" />} color="bg-[#10B981]/10 dark:bg-[#10B981]/20" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:p-6 md:gap-4 sm:p-8">
        <div className="bg-white dark:bg-[#131B2A] p-4 sm:p-6 md:p-4 sm:p-8 rounded-card shadow-soft border border-slate-100 dark:border-slate-800 w-full overflow-hidden">
            <div className="flex items-center justify-between mb-6">
                <h3 className="text-xs md:text-sm font-semibold text-slate-900 dark:text-white uppercase tracking-wider">{t('dashboard.statusDistribution', 'Distribuição de Status')}</h3>
                <span className="text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 font-bold px-2 py-1 rounded-md">{t('dashboard.shortcuts', 'Atalhos')}</span>
            </div>
        ...
        <div className="bg-white dark:bg-[#131B2A] p-4 sm:p-6 md:p-4 sm:p-8 rounded-card shadow-soft border border-slate-100 dark:border-slate-800 w-full overflow-hidden">
            <div className="flex items-center justify-between mb-6">
                <h3 className="text-xs md:text-sm font-semibold text-slate-900 dark:text-white uppercase tracking-wider">{t('dashboard.loadBalance', 'Equilíbrio de Carga')}</h3>
                <span className="text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 font-bold px-2 py-1 rounded-md">{t('dashboard.percentage', 'Porcentagem')}</span>
            </div>
