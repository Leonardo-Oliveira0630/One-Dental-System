  ...
  const handleSaveAndAccept = async () => {
    ...
    try {
      const compiledPolicies: SupplierStorePolicies = {
        deliveryPolicy: {
          defaultDispatchDays: Number(dispatchDays) || 2,
          shippingModes: shippingModes || ['FRENET', 'CORREIOS', 'CARRIER'],
          freeShippingEnabled: !!freeShippingEnabled,
          ...(freeShippingEnabled && freeShippingThreshold ? { freeShippingThreshold: Number(freeShippingThreshold) } : {}),
          ...(deliveryNotes ? { deliveryNotes } : {}),
        },
        returnPolicy: {
          legalReturnPeriodDays: Math.max(7, Number(legalReturnDays) || 7),
          returnConditions: returnConditions || '',
          reverseLogisticsCoverage: reverseLogisticsCoverage || 'SELLER_PAYS_DEFECT_AND_REPENTANCE',
          exchangeProcessingDays: Number(exchangeProcessingDays) || 3
        },
        warrantyPolicy: {
          legalWarrantyDays: Math.max(90, Number(legalWarrantyDays) || 90),
          ...(manufacturerWarrantyMonths ? { manufacturerWarrantyMonths: Number(manufacturerWarrantyMonths) } : {}),
          warrantyCoverageDetails: warrantyProcedure || '',
          ...(technicalAssistanceInfo ? { technicalAssistanceInfo } : {}),
        },
        regulatedProductsPolicy: {
          anvisaComplianceConfirmed: !!anvisaComplianceConfirmed,
          ...(techRespName ? { technicalResponsibleName: techRespName } : {}),
          ...(techRespDocType ? { technicalResponsibleDocType: techRespDocType } : {}),
          ...(techRespDocNumber ? { technicalResponsibleDocNumber: techRespDocNumber } : {}),
          ...(afeNumber ? { afeOrAnvisaAuthorizationNumber: afeNumber } : {}),
        ...
        customerServicePolicy: {
          maxResponseTimeHours: Number(maxResponseHours) || 24,
          supportChannels: supportChannels || ['PLATFORM_CHAT', 'WHATSAPP', 'EMAIL'],
        ...
        termsAcceptance: {
          ...
          ...(repName ? { legalRepresentativeName: repName } : {}),
          ...(repCpf ? { legalRepresentativeCpf: repCpf } : {}),
      ...
      await updateOrganization(currentOrg.id, {
        name: companyName || currentOrg.name,
        ...(cpfCnpj ? { cpfCnpj } : {}),
        ...(phone ? { phone } : {}),
        ...(email ? { email } : {}),
        ...(originAddress.cep ? { cep: originAddress.cep } : {}),
        ...(originAddress.street ? { address: originAddress.street } : {}),
        ...(originAddress.number ? { number: originAddress.number } : {}),
        ...(originAddress.complement ? { complement: originAddress.complement } : {}),
        ...(originAddress.neighborhood ? { neighborhood: originAddress.neighborhood } : {}),
        ...(originAddress.city ? { city: originAddress.city } : {}),
        ...(originAddress.state ? { state: originAddress.state } : {}),
        storeSettings: updatedStoreSettings,
        financialSettings: {
          ...(currentOrg.financialSettings || {}),
          ...(techRespName || repName ? { techResponsibleName: techRespName || repName } : {}),
          ...(repCpf ? { techResponsibleCpf: repCpf } : {}),
        }
      });
