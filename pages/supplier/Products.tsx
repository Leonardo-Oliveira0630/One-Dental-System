...
export const SupplierProducts = () => {
  ...
  const handleShareStore = () => {
    ...
    const shareUrl = `http://labprox.com.br/#/store?supplierId=${currentUser.organizationId}`;
  ...
  const handleShareProduct = (productId: string) => {
    if (!currentUser?.organizationId) return;
    const shareUrl = `http://labprox.com.br/#/store?supplierId=${currentUser.organizationId}&productId=${productId}`;
    navigator.clipboard.writeText(shareUrl);
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 w-full max-w-4xl rounded-2xl overflow-hidden shadow-2xl text-slate-900 flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={saveItem} className="px-4 pb-4 sm:px-6 sm:pb-6 overflow-y-auto space-y-6 flex-1">
