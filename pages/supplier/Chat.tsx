...
export const SupplierChat: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-5rem)] max-w-7xl mx-auto px-2 sm:px-4 py-3 sm:py-6">
      {/* Top Header Card */}
      <div className="bg-white dark:bg-[#0B0F17] border border-slate-200 dark:border-slate-800 rounded-2xl p-4 sm:p-5 mb-4 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {totalUnreadCount > 0 && (
                <span className="px-2.5 py-0.5 bg-indigo-100 dark:bg-indigo-950/80 text-indigo-700 dark:text-indigo-300 font-bold text-xs rounded-full animate-pulse">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <button
            ...
            className="flex-1 sm:flex-initial px-3.5 py-2 bg-slate-100 dark:bg-[#0E1626] hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 font-bold text-xs rounded-xl transition-all border border-slate-200 dark:border-slate-800 flex items-center justify-center gap-1.5 cursor-pointer"
          >
            <Package size={14} />
      ...
      <div className="flex-1 bg-white dark:bg-[#0B0F17] border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xs flex flex-col md:flex-row min-h-0">
        {/* LEFT COLUMN: Conversations List */}
        <div className={`w-full md:w-80 lg:w-96 border-r border-slate-200 dark:border-slate-800 flex flex-col bg-slate-50/50 dark:bg-[#080C14] h-full ${
          mobilePane === 'CHAT' ? 'hidden md:flex' : 'flex'
        }`}>
          {/* Search Box */}
          <div className="p-3 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0B0F17]">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-50 dark:bg-[#0E1626] border border-slate-200 dark:border-slate-700 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 outline-none focus:border-indigo-600 focus:bg-white dark:focus:bg-[#0E1626] transition-all font-medium"
              />
            </div>
          </div>

          {/* Quick Filter Tabs */}
          <div className="flex items-center gap-1 p-2 border-b border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-[#0B0F17] overflow-x-auto text-[11px] font-bold">
            <button
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800/60">
            {filteredConversations.length === 0 ? (
              <div className="p-8 text-center space-y-2">
                <div className="w-12 h-12 rounded-full bg-slate-100 dark:bg-[#0E1626] text-slate-400 flex items-center justify-center mx-auto">
                  <MessageSquare size={22} />
                </div>
                <p className="text-xs font-bold text-slate-800 dark:text-slate-200">Nenhuma conversa encontrada</p>
              ...
              filteredConversations.map((conv) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3.5 transition-all flex items-start gap-3 hover:bg-white dark:hover:bg-[#0E1626] cursor-pointer ${
                      isActive ? 'bg-white dark:bg-[#0E1626] border-l-4 border-l-indigo-600 shadow-xs' : ''
                    }`}
                  >
                    <div className="w-10 h-10 rounded-xl bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 flex items-center justify-center font-bold text-xs shrink-0 mt-0.5">
                      <User size={18} />
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {conv.orderId && (
                        <div className="flex items-center gap-1 text-[10px] font-bold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/60 px-1.5 py-0.5 rounded-md mt-1 w-fit border border-indigo-100 dark:border-indigo-900/50">
                          <Package size={10} />
                          <span>Pedido #{conv.orderId.substring(0, 8).toUpperCase()}</span>
                        </div>
                      )}
                      {conv.productName && !conv.orderId && (
                        <div className="flex items-center gap-1 text-[10px] font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-[#0E1626] px-1.5 py-0.5 rounded-md mt-1 truncate max-w-[200px] border border-slate-200 dark:border-slate-800">
                          <ShoppingBag size={10} />
        ...
        <div className={`flex-1 flex-col h-full bg-white dark:bg-[#0B0F17] ${
          mobilePane === 'LIST' ? 'hidden md:flex' : 'flex'
        }`}>
          {activeConv ? (
            <>
              {/* Active Conversation Header */}
              <div className="px-4 sm:px-6 py-3.5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-[#0E1626] flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  {/* Mobile Back Button */}
                  <button
                    type="button"
                    onClick={() => setMobilePane('LIST')}
                    className="md:hidden p-2 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200/70 dark:hover:bg-slate-800 rounded-xl transition-all"
                  >
                    <ArrowLeft size={18} />
                  </button>

                  <div className="w-10 h-10 rounded-2xl bg-slate-900 dark:bg-indigo-600 text-white flex items-center justify-center font-bold text-sm shrink-0">
                    <User size={20} />
                  </div>
                ...
                {activeConv.orderId && (
                  <div className="shrink-0">
                    <button
                      ...
                      className="px-3 py-1.5 bg-indigo-50 dark:bg-indigo-950/60 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer shadow-xs"
                    >
                      <Package size={13} />
              ...
              {(activeConv.orderId || activeConv.productName) && (
                <div className="px-4 sm:px-6 py-2.5 bg-indigo-50/50 dark:bg-indigo-950/30 border-b border-indigo-100 dark:border-indigo-900/50 flex items-center justify-between text-xs text-indigo-950 dark:text-indigo-200">
                  <div className="flex items-center gap-2 truncate">
                    {activeConv.orderId ? (
                      <>
                        <Package size={15} className="text-indigo-600 dark:text-indigo-400 shrink-0" />
                      ...
                      <>
                        <ShoppingBag size={15} className="text-indigo-600 dark:text-indigo-400 shrink-0" />
                        <span className="truncate">
                  ...
                  <span className="text-[10px] font-bold text-indigo-700 dark:text-indigo-300 bg-white dark:bg-[#0B0F17] px-2 py-0.5 rounded-md border border-indigo-200 dark:border-indigo-800 shrink-0 ml-2">
                    Origem Loja Online
                  </span>
                </div>
              )}
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 bg-slate-50/40 dark:bg-[#070A10]">
                {/* Security Tag */}
                <div className="text-center">
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-white dark:bg-[#0E1626] text-slate-500 dark:text-slate-400 rounded-full text-[11px] font-medium border border-slate-200 dark:border-slate-800 shadow-2xs">
                ...
                {messages.length === 0 ? (
                  <div className="text-center py-12 space-y-2">
                    <p className="text-xs font-bold text-slate-700 dark:text-slate-300">Nenhuma mensagem nesta conversa.</p>
                        ...
                        <div
                          className={`max-w-[85%] sm:max-w-md rounded-2xl p-3.5 text-xs leading-relaxed space-y-2 shadow-xs ${
                            isMyMessage
                              ...
                              : 'bg-white dark:bg-[#0E1626] border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 rounded-tl-xs'
                          }`}
                        >
                          {msg.imageUrl && (
                            <img
              ...
              <div className="px-4 py-2 border-t border-slate-100 dark:border-slate-800/80 bg-white dark:bg-[#0B0F17] flex items-center gap-1.5 overflow-x-auto">
                ...
                {cannedReplies.map((reply, i) => (
                  <button
                    ...
                    className="px-2.5 py-1 bg-slate-50 dark:bg-[#0E1626] hover:bg-indigo-50 dark:hover:bg-indigo-950/60 text-slate-600 dark:text-slate-300 hover:text-indigo-700 dark:hover:text-indigo-300 border border-slate-200 dark:border-slate-800 hover:border-indigo-200 dark:hover:border-indigo-800 rounded-lg text-[11px] whitespace-nowrap transition-all shrink-0 cursor-pointer font-medium"
                  >
                    {reply}
                  </button>
                ))}
              ...
              {/* Message Input Box */}
              <div className="p-3 sm:p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0B0F17]">
                {showImageInput && (
                  <div className="mb-3 p-2 bg-slate-50 dark:bg-[#0E1626] rounded-xl border border-slate-200 dark:border-slate-800 flex gap-2 animate-in fade-in">
                    <input
                      type="url"
                      placeholder="Cole o link/URL da imagem ou comprovante..."
                      value={imageUrlInput}
                      onChange={(e) => setImageUrlInput(e.target.value)}
                      className="flex-1 bg-white dark:bg-[#080C14] border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5 text-xs text-slate-900 dark:text-white outline-none focus:border-indigo-600"
                    />
                    <button
                      type="button"
                      onClick={() => setShowImageInput(false)}
                      className="px-2 text-slate-400 hover:text-slate-900 dark:hover:text-white text-xs font-bold cursor-pointer"
                    >
                      ✕ Fechar
                    </button>
                  </div>
                ...
                <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                  <button
                    ...
                    className={`p-2.5 rounded-xl border transition-all cursor-pointer ${
                      showImageInput
                        ...
                        : 'bg-slate-50 dark:bg-[#0E1626] text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 border-slate-200 dark:border-slate-800'
                    }`}
                    title="Anexar Imagem"
                  >
                    <Paperclip size={16} />
                  ...
                  <input
                    ...
                    className="flex-1 bg-slate-50 dark:bg-[#0E1626] border border-slate-200 dark:border-slate-700 rounded-2xl px-4 py-3 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 outline-none focus:border-indigo-600 focus:bg-white dark:focus:bg-[#0E1626] transition-all font-medium"
                  />
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center space-y-3 bg-slate-50/50 dark:bg-[#070A10]">
              <div className="w-16 h-16 rounded-full bg-slate-100 dark:bg-[#0E1626] text-slate-400 flex items-center justify-center">
                <MessageSquare size={28} />
              </div>
              <h3 className="font-bold text-slate-900 dark:text-white text-sm">Selecione uma conversa para atender</h3>
