...
export const SupplierCoupons = () => {
  ...
  return (
    <div className="flex-1 p-6 space-y-6 bg-slate-950 text-slate-100 min-h-screen overflow-y-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-slate-900 border border-slate-800 rounded-2xl">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Crie e gerencie cupons de desconto para sua loja</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          <p className="text-slate-400">Carregando cupons...</p>
        ) : coupons.length === 0 ? (
          <div className="col-span-full p-12 text-center border-2 border-dashed border-slate-800 rounded-2xl">
            ...
            <h3 className="text-lg font-medium text-slate-300">Nenhum cupom criado</h3>
            <p className="text-slate-500 mt-1">Crie seu primeiro cupom de desconto para atrair clientes.</p>
          </div>
        ) : (
          coupons.map(coupon => (
            <div key={coupon.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative group">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-2">
                  <Ticket className="w-5 h-5 text-indigo-400" />
                  <span className="font-mono text-lg font-bold text-slate-200">{coupon.code}</span>
                </div>
                <button
                  onClick={() => toggleActive(coupon)}
                  className={`px-2 py-1 text-xs font-medium rounded-md \${coupon.active ? 'bg-emerald-500/10 text-emerald-400' : 'bg-slate-800 text-slate-400'}`}
                >
              ...
              <div className="space-y-2 mb-4">
                ...
                <p className="text-sm text-slate-400">Usos: {coupon.usedCount} {coupon.maxUses ? `/ ${coupon.maxUses}` : '(Ilimitado)'}</p>
                {coupon.applicableProductIds && coupon.applicableProductIds.length > 0 && (
                  <p className="text-xs text-indigo-400 bg-indigo-400/10 px-2 py-1 rounded inline-block">
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1">Código do Cupom</label>
                <input
                  type="text"
                  required
                  value={code}
                  onChange={e => setCode(e.target.value.toUpperCase())}
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2 text-white uppercase"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-1">Tipo de Desconto</label>
                  <select
                    value={discountType}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1">Limite de Usos (Opcional)</label>
                <input
                  type="number"
                  min="1"
                  value={maxUses}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1">Produtos Específicos (Opcional)</label>
                ...
                <div className="max-h-32 overflow-y-auto space-y-1 bg-slate-950 p-2 rounded-lg border border-slate-800">
                  {myProducts.map(product => (
                    <label key={product.id} className="flex items-center gap-2 p-1 hover:bg-slate-900 rounded cursor-pointer">
                      <input
                        type="checkbox"
                        ...
                        className="rounded border-slate-700 bg-slate-800 text-indigo-500 focus:ring-indigo-500 focus:ring-offset-slate-950"
                      />
