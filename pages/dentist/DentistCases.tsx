...
export const DentistCases = () => {
  ...
  const renderOriginBadge = (origin: 'INTERNAL' | 'REQUISITION' | 'STORE') => {
    switch (origin) {
      case 'INTERNAL':
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300 border border-blue-200/60 dark:border-blue-800/50">
            <FileText size={12} />
            Trabalho Interno
          </span>
        );
      case 'REQUISITION':
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-purple-50 dark:bg-purple-950/50 text-purple-700 dark:text-purple-300 border border-purple-200/60 dark:border-purple-800/50">
      ...
      case 'STORE':
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 border border-emerald-200/60 dark:border-emerald-800/50">
            <ShoppingBag size={12} />
            Loja Online
          </span>
        );
  ...
  const renderStatusBadge = (status: JobStatus | 'PENDING' | 'ACCEPTED' | 'REJECTED') => {
    switch (status) {
      case JobStatus.IN_PROGRESS:
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-amber-50 dark:bg-amber-950/50 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800/50">
            <Activity size={12} className="animate-spin text-amber-600 dark:text-amber-400" />
            Em Produção
          </span>
        );
      case JobStatus.WAITING_APPROVAL:
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-rose-50 dark:bg-rose-950/50 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800/50 animate-pulse">
            <ShieldAlert size={12} />
      ...
      case 'ACCEPTED':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/50">
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800/50">
            <Clock size={12} />
            Pendente
          </span>
        );
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50/50 dark:bg-[#0B0F17] p-4 md:p-4 sm:p-8 space-y-6">
      {/* HEADER SECTION */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-[#131B2A] p-4 sm:p-6 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white tracking-tight">Meus Casos</h1>
        ...
        <div className="w-full md:w-auto flex items-center gap-2 bg-slate-100/80 dark:bg-[#0E1626] p-1.5 rounded-2xl border border-slate-200 dark:border-slate-800">
          <Building size={18} className="text-slate-500 dark:text-slate-400 ml-2 shrink-0" />
          <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider hidden sm:inline">Lab:</span>
          <select
            value={selectedLabId}
            onChange={(e) => {
            ...
            className="w-full md:w-64 bg-white dark:bg-[#131B2A] border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-sm font-bold text-slate-800 dark:text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            id="lab-selector-dropdown"
          >
            {availableLabs.map((lab) => (
              <option key={lab.id} value={lab.id} className="dark:bg-[#131B2A] dark:text-white">
                {lab.name}
              </option>
            ))}
          </select>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div 
          ...
          className="bg-white dark:bg-[#131B2A] p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm cursor-pointer hover:border-blue-300 dark:hover:border-blue-500 transition-all group"
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total de Casos</span>
            <Layers size={18} className="text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors" />
          </div>
          <div className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white mt-2">{stats.total}</div>
          <span className="text-[10px] text-slate-400 font-medium">No laboratório selecionado</span>
        </div>

        <div 
          onClick={() => { setStatusFilter('ACTIVE'); }}
          className="bg-white dark:bg-[#131B2A] p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm cursor-pointer hover:border-amber-300 dark:hover:border-amber-500 transition-all group"
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-amber-700 dark:text-amber-400 uppercase tracking-wider">Em Produção</span>
            <Activity size={18} className="text-amber-500 group-hover:animate-spin" />
          </div>
          <div className="text-2xl md:text-3xl font-black text-amber-600 dark:text-amber-400 mt-2">{stats.active}</div>
          <span className="text-[10px] text-slate-400 font-medium">Trabalhos em andamento</span>
        </div>

        <div 
          onClick={() => { setStatusFilter('PENDING'); }}
          className="bg-white dark:bg-[#131B2A] p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm cursor-pointer hover:border-rose-300 dark:hover:border-rose-500 transition-all group"
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-rose-700 dark:text-rose-400 uppercase tracking-wider">Aguardando Aprovação</span>
            <ShieldAlert size={18} className="text-rose-500" />
          </div>
          <div className="text-2xl md:text-3xl font-black text-rose-600 dark:text-rose-400 mt-2">{stats.waitingApproval}</div>
          <span className="text-[10px] text-slate-400 font-medium">Aprovações e autorizações</span>
        </div>

        <div 
          onClick={() => { setStatusFilter('COMPLETED'); }}
          className="bg-white dark:bg-[#131B2A] p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm cursor-pointer hover:border-emerald-300 dark:hover:border-emerald-500 transition-all group"
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider">Concluídos / Entregues</span>
            <CheckCircle2 size={18} className="text-emerald-500" />
          </div>
          <div className="text-2xl md:text-3xl font-black text-emerald-600 dark:text-emerald-400 mt-2">{stats.completed}</div>
          <span className="text-[10px] text-slate-400 font-medium">Casos finalizados</span>
        </div>
      </div>

      {/* FILTERS & SEARCH BAR */}
      <div className="bg-white dark:bg-[#131B2A] p-4 md:p-4 sm:p-6 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="relative w-full md:w-full sm:w-96 max-w-[calc(100vw-2rem)]">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-[#0E1626] border border-slate-200 dark:border-slate-700 rounded-2xl text-sm font-medium text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white dark:focus:bg-[#0E1626] transition-all"
          ...
          <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-[#0E1626] p-1 rounded-2xl w-full md:w-auto overflow-x-auto border border-transparent dark:border-slate-800">
            <button
              onClick={() => setOriginFilter('ALL')}
              className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
                originFilter === 'ALL' ? 'bg-white dark:bg-slate-800 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
              }`}
            >
              Todos os Origens
            </button>
            <button
              ...
              className={`flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
                originFilter === 'INTERNAL' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
              }`}
            >
              <FileText size={12} />
              Trabalhos Internos
            ...
            <button
              ...
              className={`flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
                originFilter === 'REQUISITION' ? 'bg-purple-600 text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
              }`}
            >
              <ClipboardList size={12} />
              Requisições
            ...
            <button
              ...
              className={`flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
                originFilter === 'STORE' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
              }`}
            >
              <ShoppingBag size={12} />
              Loja Online
        ...
        {/* STATUS TAB FILTER */}
        <div className="flex items-center gap-2 border-t border-slate-100 dark:border-slate-800 pt-3 flex-wrap">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-wider mr-2">Status:</span>
          <button
            onClick={() => setStatusFilter('ACTIVE')}
            className={`px-3 py-1 rounded-lg text-xs font-bold transition-all ${
              statusFilter === 'ACTIVE' 
                ? 'bg-amber-500/10 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 border border-amber-300 dark:border-amber-800/60' 
          ...
          <button
            ...
            className={`px-3 py-1 rounded-lg text-xs font-bold transition-all ${
              statusFilter === 'PENDING' 
                ? 'bg-rose-500/10 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 border border-rose-300 dark:border-rose-800/60' 
                : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'
            }`}
          >
            Aguardando Aprovação
          ...
          <button
            ...
            className={`px-3 py-1 rounded-lg text-xs font-bold transition-all ${
              statusFilter === 'COMPLETED' 
                ? 'bg-emerald-500/10 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800/60' 
                : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'
            }`}
          >
            Concluídos
      ...
      {/* CASES LIST / TABLE */}
      <div className="bg-white dark:bg-[#131B2A] rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center p-16 text-center space-y-4">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-800 dark:text-white">Nenhum caso encontrado</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 max-w-sm mt-1">
                Não há trabalhos ou requisições cadastradas em seu nome no laboratório <strong className="text-slate-700 dark:text-slate-200">{selectedLab.name}</strong> para o filtro selecionado.
              </p>
            </div>
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            ...
            <div className="hidden md:grid grid-cols-12 gap-4 px-6 py-3.5 bg-slate-50 dark:bg-[#0E1626] text-[11px] font-black uppercase text-slate-400 tracking-wider">
            ...
            {filteredCases.map((c) => (
              <div
                key={c.id}
                className="p-4 md:px-6 md:py-4 hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors flex flex-col md:grid md:grid-cols-12 gap-3 md:gap-4 items-start md:items-center group"
              >
                {/* OS Number & Patient */}
                <div className="col-span-2 flex items-center gap-2">
                  <button
                    ...
                    className="font-mono text-sm font-black text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 hover:underline flex items-center gap-1"
                  >
                    {c.osNumber}
                  </button>
                </div>
                ...
                <div className="col-span-3">
                  ...
                  <div className="text-xs text-slate-500 dark:text-slate-400 font-medium truncate mt-0.5">
                    {c.servicesText}
                    {c.teethText && <span className="text-blue-600 dark:text-blue-400 font-semibold ml-1">(Dentes: {c.teethText})</span>}
                  </div>
                </div>
                ...
                {/* Dates */}
                <div className="col-span-2 text-xs text-slate-500 dark:text-slate-400 font-medium space-y-0.5">
                ...
                <div className="col-span-1 flex justify-end">
                  <button
                    ...
                    className="p-2 bg-slate-100 dark:bg-slate-800 hover:bg-blue-600 hover:text-white dark:hover:bg-blue-600 text-slate-600 dark:text-slate-300 rounded-xl transition-all flex items-center gap-1 text-xs font-bold group-hover:shadow-md"
      ...
      {selectedCase && (
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#131B2A] rounded-3xl max-w-lg w-full p-4 sm:p-6 shadow-2xl border border-slate-100 dark:border-slate-800 space-y-6 relative animate-in zoom-in-95 duration-200">
            {/* CLOSE BUTTON */}
            <button
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">
                Laboratório: <strong className="text-slate-800 dark:text-slate-200">{selectedCase.labName}</strong>
              </p>
            </div>

            {/* CASE DETAILS */}
            <div className="bg-slate-50 dark:bg-[#0E1626] p-4 rounded-2xl border border-slate-200/80 dark:border-slate-800 space-y-3">
              <div>
                <span className="text-[10px] font-black uppercase text-slate-400 tracking-wider">Paciente</span>
                <p className="text-sm font-bold text-slate-800 dark:text-white flex items-center gap-1.5 mt-0.5">
              ...
              {selectedCase.currentSector && (
                <div>
                  ...
                  <p className="text-xs font-bold text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/50 px-2.5 py-1 rounded-lg border border-amber-200/60 dark:border-amber-800/60 w-fit mt-1 flex items-center gap-1">
                    <Activity size={12} className="animate-spin text-amber-600 dark:text-amber-400" />
                    {selectedCase.currentSector}
                  </p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-200/60 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-400">
                <div>
                  <span className="text-[10px] font-black uppercase text-slate-400">Data de Entrada</span>
                  <p className="font-medium mt-0.5 text-slate-800 dark:text-slate-200">{selectedCase.createdAt.toLocaleDateString('pt-BR')}</p>
                </div>
                {selectedCase.dueDate && (
                  <div>
                    <span className="text-[10px] font-black uppercase text-slate-400">Previsão de Entrega</span>
                    <p className="font-bold text-blue-600 dark:text-blue-400 mt-0.5">{selectedCase.dueDate.toLocaleDateString('pt-BR')}</p>
                  </div>
                )}
              </div>
            </div>
