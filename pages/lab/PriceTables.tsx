...
export const PriceTables = () => {
    ...
    if (!hasPerm('catalog:prices_view')) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-slate-500">
                ...
                <h2 className="text-xl font-black uppercase tracking-widest">{t('common.accessDenied', 'Acesso Negado')}</h2>
                <p className="text-sm">{t('prices.accessDeniedMsg', 'Você não tem permissão para gerenciar tabelas de preços.')}</p>
            </div>
        );
    }
    ...
    const handleSave = async () => {
        if (!tableName.trim()) return alert(t('prices.nameRequiredAlert', "Dê um nome para a tabela"));
        ...
        try {
            ...
            console.error(error);
            alert(t('prices.saveErrorAlert', "Erro ao salvar tabela de preços"));
        }
    };

    const handleDelete = async (id: string) => {
        if (window.confirm(t('prices.deleteConfirm', "Deseja realmente excluir esta tabela de preços?"))) {
                ...
                {priceTables.map(table => (
                    <div key={table.id} className="bg-white px-4 pb-4 sm:px-6 sm:pb-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all group">
                        ...
                        <h3 className="font-black text-slate-800 text-lg uppercase mb-1 flex items-center gap-2">
                            {table.name}
                            {table.isDefault && (
                                <span className="bg-emerald-100 text-emerald-800 text-[9px] px-2 py-0.5 rounded-full font-black uppercase tracking-wider">{t('prices.defaultBadge', 'Padrão')}</span>
                            )}
                        </h3>
                        <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">{Object.keys(table.prices).length} {t('prices.customServices', 'Serviços Customizados')}</p>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-in zoom-in duration-200 overflow-hidden">
                        <div className="p-4 sm:p-8 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center bg-slate-50 gap-4">
                            <div>
                                <h3 className="text-2xl font-black text-slate-800 uppercase tracking-tight">{editingTable ? t('prices.editTableTitle', 'Editar Tabela') : t('prices.newTableTitle', 'Nova Tabela de Preços')}</h3>
                                <p className="text-xs text-slate-500 font-black uppercase tracking-widest mt-1">{t('prices.configBaseValues', 'Configure o valor base e variações para cada serviço')}</p>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-8">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">{t('prices.tableNameLabel', 'Nome da Tabela')}</label>
                                <input 
                                    value={tableName}
                                    onChange={e => setTableName(e.target.value)}
                                    placeholder={t('prices.tableNamePlaceholder', 'Ex: Tabela Convênio A, Tabela Especial 2024...')}
                            ...
                            <div className="space-y-6">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                    <h4 className="text-sm font-black text-slate-800 uppercase tracking-tight flex items-center gap-2">
                                        <Layers size={18} className="text-blue-500" /> {t('prices.priceConfig', 'Configuração de Preços')}
                                    </h4>
                                    <div className="relative w-full sm:w-64">
                                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                                        <input 
                                            placeholder={t('prices.filterServices', 'Filtrar serviços...')}
                                            value={searchTerm}
                                            ...
                                            <div key={jt.id} className="bg-white border border-slate-100 rounded-[32px] overflow-hidden shadow-sm hover:border-blue-100 transition-all">
                                                <div className="px-4 pb-4 sm:px-6 sm:pb-6 bg-slate-50/50 flex flex-col sm:flex-row justify-between sm:items-center border-b border-slate-50 gap-4">
                                                    <div>
                                                        ...
                                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{t('prices.originalPrice', 'Preço Original')}: R$ {jt.basePrice.toFixed(2)}</p>
                                                    </div>
                                                    <div className="flex items-center gap-3">
                                                        <span className="text-[10px] font-black text-slate-400 uppercase">{t('prices.tablePrice', 'Preço Tabela (R$)')}</span>
                                                        <input 
                                                            type="number"
                                                ...
                                                {((jt.variationGroups && jt.variationGroups.length > 0) || (jt.variations && jt.variations.length > 0)) && (
                                                    <div className="px-4 pb-4 sm:px-6 sm:pb-6 bg-white space-y-4">
                                                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] px-1">{t('prices.variationsAdditional', 'Variações / Adicionais')}</p>
                                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
                                                            {(jt.variationGroups && jt.variationGroups.length > 0 ? jt.variationGroups : [{ id: 'default', name: t('prices.optionsDefault', 'Opções'), options: jt.variations || [] }]).map((group: any) => (
                        ...
                        <div className="p-4 sm:p-8 border-t bg-slate-50 flex justify-end gap-4">
                            <button onClick={() => setIsModalOpen(false)} className="px-8 py-4 font-black text-slate-400 hover:text-slate-600 hover:bg-slate-200 rounded-2xl transition-all uppercase text-xs">{t('common.cancel', 'Cancelar')}</button>
                            <button 
                                onClick={handleSave}
                            ...
                            >
                                <Save size={20} /> {t('prices.saveTable', 'Salvar Tabela')}
                            </button>
                        </div>
                    </div>
                </div>
