...
export const SupplierStoreChatModal: React.FC<SupplierStoreChatModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-center justify-center p-2 sm:p-6 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div 
        ...
        <div className="w-full md:w-80 lg:w-96 border-r border-zinc-200 flex flex-col bg-zinc-50/70 h-full">
          ...
          <div className="p-4 border-b border-zinc-200 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] text-zinc-500 font-medium">
                  {isSupplierOrg ? 'Atendimento a Laboratórios & Dentistas' : 'Dúvidas, Pedidos & Pós-Venda'}
                </p>
              </div>
            </div>
            ...
            <button
              type="button"
              onClick={onClose}
              className="md:hidden p-2 text-zinc-400 hover:text-zinc-900 rounded-xl"
            >
              <X size={18} />
            </button>
          </div>
          ...
          {activeConv ? (
            <>
              ...
              {(activeConv.orderId || activeConv.productName) && (
                <div className="px-6 py-2.5 bg-zinc-100/80 border-b border-zinc-200/70 flex items-center justify-between text-xs text-zinc-700">
                  <div className="flex items-center gap-2">
                    {activeConv.orderId ? (
                      <>
                        <Package size={14} className="text-zinc-600" />
                        <span>Conversando sobre o <strong>Pedido #{activeConv.orderId.substring(0, 8).toUpperCase()}</strong></span>
                      </>
                    ) : (
                      <>
                        <ShoppingBag size={14} className="text-zinc-600" />
                        <span>Produto de Referência: <strong>{activeConv.productName}</strong></span>
                      </>
                    )}
                  </div>
                  ...
                  messages.map((msg) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className={`max-w-[80%] sm:max-w-md rounded-2xl p-3.5 text-xs leading-relaxed space-y-2 shadow-xs ${
                            isMyMessage
              ...
              <div className="p-4 border-t border-zinc-200 bg-white">
                {showImageInput && (
                  <div className="mb-3 p-2 bg-zinc-50 rounded-xl border border-zinc-200 flex gap-2 animate-in fade-in">
                    ...
                    <button
                      ...
                      className="px-2 text-zinc-400 hover:text-zinc-900 text-xs font-bold"
                    >
                      Fechar
                    </button>
                  </div>
                ...
                <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="p-3 bg-zinc-950 hover:bg-zinc-800 disabled:opacity-40 text-white rounded-2xl transition-all shadow-xs shrink-0 flex items-center justify-center"
                  >
                    <Send size={16} />
                  </button>
                </form>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center space-y-3 bg-zinc-50/50">
